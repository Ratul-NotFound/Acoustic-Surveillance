"""
build_dataset_methodology_doc.py
─────────────────────────────────
Generates a comprehensive, highly detailed technical document for the Dataset Methodology,
Sourcing, Speech Purging, Physics Synthesis, ISO 9613-2 Augmentation, and Grouping.

Exports:
  - E:\\software\\acoustic-surveillance\\results\\dataset_methodology_detailed_report.md
  - E:\\software\\acoustic-surveillance\\results\\dataset_methodology_detailed_report.doc
  - E:\\software\\acoustic-surveillance\\results\\dataset_methodology_detailed_report.html
"""
...
md_content = """# Technical Report: Acoustic Dataset Sourcing, Quality Control, Physical Synthesis, and Augmentation Methodology
...
$$s(t) = A \cdot e^{-\alpha t} \cdot \sin(2\pi f_c t) + N(t) \cdot e^{-\beta t}$$
Where $\alpha = 80$ controls the sharp peak (<30ms) and $\beta = 15$ governs environmental echo.
...
$$s(t) = A \cdot \exp\left(-\frac{t}{\tau}\right) \cdot N_{\text{bandpass}}(t)$$
...
$$s_{\text{mixed}}(t) = s_{\text{threat}}(t) + \alpha \cdot s_{\text{noise}}(t)$$
Where $\alpha$ is dynamically scaled such that $\text{SNR}_{\text{dB}} \in [-5\text{ dB}, +15\text{ dB}]$.
...
- **Directory Location**: `E:\software\acoustic-surveillance\data_prep\q1_dataset\`
...
# Write MD
md_path = os.path.join(RESULTS_DIR, "dataset_methodology_detailed_report.md")
...
<head>
<meta charset="utf-8">
<title>Dataset Methodology Detailed Report</title>
<style>
...
h2 {{ color: #2c5282; margin-top: 25px; border-bottom: 1px solid #e2e8f0; padding-bottom: 4px; }}
h3 {{ color: #2b6cb0; }}
...
html_path = os.path.join(RESULTS_DIR, "dataset_methodology_detailed_report.html")
doc_path = os.path.join(RESULTS_DIR, "dataset_methodology_detailed_report.doc")
...
print("Comprehensive Dataset Methodology Documentation generated successfully!")
